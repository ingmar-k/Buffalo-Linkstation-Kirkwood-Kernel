...
static int eth_ufo_tx(struct sk_buff *skb, struct net_device *dev, int txq)
{
	...
	while (left) {
		...
		while (fn < nr) {
			...
			data = page_address(fp->page.p) + fp->page_offset;
...
static const struct net_device_ops mv_eth_gtw_netdev_ops = {
	...
	.ndo_set_rx_mode	= mv_gtw_set_multicast_list, 
...
void print_skb(struct sk_buff* skb)
{
    ...
    for(i=0; i<skb_shinfo(skb)->nr_frags; i++)
    {
        printk("\t frag_%d. page=%p, page_offset=0x%x, size=%d\n",
            i, page_address(skb_shinfo(skb)->frags[i].page.p), 
...
static int __init mv_eth_init_module( void ) 
{
	...
	/* mvBoardIsSwitchConnected may return MV_ERROR(!=0) */
	if (mvBoardIsSwitchConnected(port) != 1)
		continue;
